...
- [ ] 锚点：`y=2+0.5a+1.5u` → 系数/截距误差 ≤1e-10；`y=1+u+0.5u²` 预测 ≤1e-4（poly 走岭回归 λ=1e-5，源 C# 测试容差即 1e-4；实测 maxErr 9.17e-6）；15 特征 → 报错
...
| 输出数 | 20 | 10 | 报告宽度 |
| poly 项数 | 100 | 100 | 保持 |
| max_starts | 50 | 20（默认 10，与 §4.2 UDF 契约及 C# 一致） | 10×4000 评估在 VBA 可能秒级到十秒级 |
| 单起点评估 | 4000 | 2000 | 同上 |
...
5. `max_starts` 默认 10 与规模上限（§6）是否接受（已按 C# 契约统一为 10）
